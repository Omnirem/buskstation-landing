...
const WaitlistFormDialog: React.FC<WaitlistFormDialogProps> = ({
  ...
  const onSubmit: SubmitHandler<WaitlistFormValues> = async (data) => {
    try {
      ...
      toast({
        ...
        description: result.message || "You've been added to the waitlist. We'll notify you at buskstation@gmail.com!",
        variant: 'default',
      });
      reset();
      setIsOpen(false);
      ...
      toast({
        ...
        description: errorMessage || 'Could not submit your details. Please try again later.',
        variant: 'destructive',
      });
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogTrigger asChild>
        <button
          className={cn(
            ...
            'bg-foreground text-background hover:bg-foreground/90', // Updated style to match "Contact Us"
            triggerClassName
          )}
      ...
      <DialogContent className="sm:max-w-[480px] bg-card/30 backdrop-blur-xl border-border/20 shadow-xl rounded-2xl">
        <DialogHeader className="pt-2">
          <DialogTitle className="font-headline text-2xl text-primary">Join Our Waitlist</DialogTitle>
          <DialogDescription className="text-foreground/80">
            Be the first to know when Busk Station launches. Enter your details below!
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2 pb-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              <Label htmlFor="waitlist-firstName" className="text-foreground/90 font-medium">First Name</Label>
              <Input
            ...
            <div className="space-y-1">
              <Label htmlFor="waitlist-lastName" className="text-foreground/90 font-medium">Last Name</Label>
              <Input
          ...
          <div className="space-y-1">
            <Label htmlFor="waitlist-email" className="text-foreground/90 font-medium">Email Address</Label>
            <Input
              id="waitlist-email"
              type="email"
              {...register('email')}
